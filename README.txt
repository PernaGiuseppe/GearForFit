...
3 DOCUMENTAZIONE API BASE_URL (http://localhost:3001)
3.1 Autenticazione
📤 POST /auth/register
Crea nuovo account.
...
Si inserisce nella path la dieta standard di riferimento. Tramite il calcolo BMR, la dieta custom viene generata. is_standard sarà false e is_attiva sarà true.
...
Auth: bearer token (GOLD in su)
Crea una scheda di allenamento personalizzata. is_standard sarà false e is_attiva sarà true.
...
📤 POST /admin/diete
Auth: bearer token (ADMIN)
Crea una dieta standard. is_standard sarà true.
...
Auth: bearer token (ADMIN)
Crea una scheda di allenamento standard. is_standard sarà true. Stesso body delle schede custom per utente.
